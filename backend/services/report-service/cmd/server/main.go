...
func main() {
	cfg, _ := config.Load("")
	cfg.Server.Name = "report-service"
	if cfg.Server.Port == 0 || cfg.Server.Port == 8080 { cfg.Server.Port = 8093 }
	...
	rh := handler.NewReportHandler(reportAppService)
	...
	rh.RegisterRoutes(engine.Group("/api/v1/report"))

	addr := fmt.Sprintf("%s:%d", cfg.Server.Host, cfg.Server.Port)
	srv := &http.Server{Addr: addr, Handler: engine, ReadTimeout: 60 * time.Second, WriteTimeout: 60 * time.Second}
	go func() {
		log.Infof("Report 报表服务启动在 %s", addr)
		if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed { log.Fatalf("Report 报表服务启动失败: %v", err) }
	}()
	quit := make(chan os.Signal, 1)
	signal.Notify(quit, syscall.SIGINT, syscall.SIGTERM)
	<-quit
	log.Info("正在关闭 Report 报表服务...")
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	if err := srv.Shutdown(ctx); err != nil { log.Errorf("Report 报表服务关闭异常: %v", err) }
	log.Info("Report 报表服务已关闭")
}
