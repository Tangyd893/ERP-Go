...
func main() {
	...
	if dbErr != nil {
		...
		log.Info("数据库连接成功")
		db = database
		appService = app.NewFileAppService(repository.NewFileRepository(db))
	}
	fh := handler.NewFileHandler(appService)
	...
	fh.RegisterRoutes(engine.Group("/api/v1/file"))

	addr := fmt.Sprintf("%s:%d", cfg.Server.Host, cfg.Server.Port)
	srv := &http.Server{Addr: addr, Handler: engine, ReadTimeout: 60 * time.Second, WriteTimeout: 60 * time.Second}
	go func() {
		log.Infof("File 文件服务启动在 %s", addr)
		if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed { log.Fatalf("File 文件服务启动失败: %v", err) }
	}()
	quit := make(chan os.Signal, 1)
	signal.Notify(quit, syscall.SIGINT, syscall.SIGTERM)
	<-quit
	log.Info("正在关闭 File 文件服务...")
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	if db != nil { if sqlDB, _ := db.DB(); sqlDB != nil { sqlDB.Close() } }
	if err := srv.Shutdown(ctx); err != nil { log.Errorf("File 文件服务关闭异常: %v", err) }
	log.Info("File 文件服务已关闭")
}
