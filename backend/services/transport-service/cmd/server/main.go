...
func main() {
	...
	if cfg.Server.Port == 0 || cfg.Server.Port == 8080 { cfg.Server.Port = 8088 }
	log := logger.New(cfg.Log.Level, cfg.Log.Format, cfg.Log.Output, cfg.Server.Name, os.Getenv("ENVIRONMENT"))

	var db *gorm.DB
	var appService *app.TransportAppService
	...
	if dbErr != nil {
		...
		log.Info("数据库连接成功")
		db = database
		appService = app.NewTransportAppService(repository.NewTransportRepository(db))
	}
	th := handler.NewTransportHandler(appService)
	...
	th.RegisterRoutes(engine.Group("/api/v1/transport"))

	addr := fmt.Sprintf("%s:%d", cfg.Server.Host, cfg.Server.Port)
	srv := &http.Server{Addr: addr, Handler: engine, ReadTimeout: 30 * time.Second, WriteTimeout: 30 * time.Second}
	go func() {
		log.Infof("TMS 物流服务启动在 %s", addr)
		if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed { log.Fatalf("TMS 物流服务启动失败: %v", err) }
	}()
	quit := make(chan os.Signal, 1)
	signal.Notify(quit, syscall.SIGINT, syscall.SIGTERM)
	<-quit
	log.Info("正在关闭 TMS 物流服务...")
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	if db != nil { if sqlDB, _ := db.DB(); sqlDB != nil { sqlDB.Close() } }
	if err := srv.Shutdown(ctx); err != nil { log.Errorf("TMS 物流服务关闭异常: %v", err) }
	log.Info("TMS 物流服务已关闭")
}
