...
func main() {
	cfg, _ := config.Load("")
	cfg.Server.Name = "purchase-service"
	if cfg.Server.Port == 0 || cfg.Server.Port == 8080 { cfg.Server.Port = 8091 }
	log := logger.New(cfg.Log.Level, cfg.Log.Format, cfg.Log.Output, cfg.Server.Name, os.Getenv("ENVIRONMENT"))

	var db *gorm.DB
	var appService *app.PurchaseAppService
	...
	if dbErr != nil {
		...
		log.Info("数据库连接成功")
		db = database
		appService = app.NewPurchaseAppService(repository.NewPurchaseRepository(db))
	}
	ph := handler.NewPurchaseHandler(appService)
	...
	ph.RegisterRoutes(engine.Group("/api/v1/purchase"))

	addr := fmt.Sprintf("%s:%d", cfg.Server.Host, cfg.Server.Port)
	srv := &http.Server{Addr: addr, Handler: engine, ReadTimeout: 30 * time.Second, WriteTimeout: 30 * time.Second}
	go func() {
		log.Infof("Purchase 采购服务启动在 %s", addr)
		if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed { log.Fatalf("Purchase 采购服务启动失败: %v", err) }
	}()
	quit := make(chan os.Signal, 1)
	signal.Notify(quit, syscall.SIGINT, syscall.SIGTERM)
	<-quit
	log.Info("正在关闭 Purchase 采购服务...")
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	if db != nil { if sqlDB, _ := db.DB(); sqlDB != nil { sqlDB.Close() } }
	if err := srv.Shutdown(ctx); err != nil { log.Errorf("Purchase 采购服务关闭异常: %v", err) }
	log.Info("Purchase 采购服务已关闭")
}
