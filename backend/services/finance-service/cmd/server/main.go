...
func main() {
	cfg, _ := config.Load("")
	cfg.Server.Name = "finance-service"
	if cfg.Server.Port == 0 || cfg.Server.Port == 8080 { cfg.Server.Port = 8092 }
	log := logger.New(cfg.Log.Level, cfg.Log.Format, cfg.Log.Output, cfg.Server.Name, os.Getenv("ENVIRONMENT"))

	var db *gorm.DB
	var appService *app.FinanceAppService
	...
	if dbErr != nil {
		...
		log.Info("数据库连接成功")
		db = database
		appService = app.NewFinanceAppService(repository.NewFinanceRepository(db))
	}
	fh := handler.NewFinanceHandler(appService)
	...
	fh.RegisterRoutes(engine.Group("/api/v1/finance"))

	addr := fmt.Sprintf("%s:%d", cfg.Server.Host, cfg.Server.Port)
	srv := &http.Server{Addr: addr, Handler: engine, ReadTimeout: 30 * time.Second, WriteTimeout: 30 * time.Second}
	go func() {
		log.Infof("Finance 财务服务启动在 %s", addr)
		if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed { log.Fatalf("Finance 财务服务启动失败: %v", err) }
	}()
	quit := make(chan os.Signal, 1)
	signal.Notify(quit, syscall.SIGINT, syscall.SIGTERM)
	<-quit
	log.Info("正在关闭 Finance 财务服务...")
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	if db != nil { if sqlDB, _ := db.DB(); sqlDB != nil { sqlDB.Close() } }
	if err := srv.Shutdown(ctx); err != nil { log.Errorf("Finance 财务服务关闭异常: %v", err) }
	log.Info("Finance 财务服务已关闭")
}
