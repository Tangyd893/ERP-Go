...
这份 README 以当前仓库内容为准。项目已完成 P0 项目启动与工程底座阶段，具备 Go 服务模板、Vue 前端骨架、Docker 中间件编排和公共组件库，正在按里程碑逐步构建业务服务。
...
- `backend/shared` 公共组件：配置加载、结构化日志、统一错误码、请求中间件、统一响应、Outbox/Inbox 基类、分页、校验。
- Go 服务模板（IAM 服务）：Gin HTTP 服务、健康检查、优雅关闭、追踪 ID 注入。
- API 网关骨架：请求代理路由、追踪 ID/请求 ID 透传。
- gRPC proto 契约目录（含 IAM 服务 proto 示例）。
- Vue 3 + TypeScript + Vite 前端工作区（npm workspaces）：
  - `admin-web`：ERP 管理后台（Element Plus、Vue Router、Pinia、登录页、主布局、首页看板）。
  - `warehouse-pda`：WMS PDA 移动端（扫码作业入口）。
  - `dashboard-web`：经营看板（数据卡片和图表占位）。
  - `shared`：共享 API 客户端和类型定义。
- Docker Compose 开发环境：PostgreSQL、Redis、RabbitMQ、MinIO、OpenSearch、Prometheus、Grafana、Jaeger。
- `.env.example` 开发配置模板。
- 完整的架构设计文档、领域模型、微服务设计、接口事件设计、数据模型设计、里程碑清单。

### 尚未完成（后续阶段）

- P1 账号租户与权限底座（IAM/Tenant 服务业务逻辑）。
- 各业务服务的数据库 migration 和业务代码。
- 自动化测试（单元测试、集成测试、契约测试、端到端测试）。
- Kubernetes 生产部署配置。
