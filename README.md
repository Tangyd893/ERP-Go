...
- 前端：历史上 `admin-web`、`warehouse-pda`、`dashboard-web` 的 typecheck 与 build 均通过；**当前部分 Windows shell 可能无法执行 npm**，需通过 `make frontend-typecheck` 或修复 Node 环境后复验。`warehouse-pda` 作业页仍为静态占位。
