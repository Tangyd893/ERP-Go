...
>
> **数据口径**：2026-06-11 刷新；SonarCloud [main](https://sonarcloud.io/summary/overall?id=Tangyd893_ERP-Go&branch=main) 最新 revision `9e6458c`（待 CI 重扫）。质量报告见 [sonarcloud-report.md](./sonarcloud-report.md)。

## 1. 当前快照

| 维度 | 状态 |
| --- | --- |
| 功能完成度 | 约 **98%**（§6 功能验收 **30/31**；P0–P9 全流程闭环：波次/物流/面单、采购入库、财务结算、渠道适配器、部署 runbook） |
| 发布就绪度 | 约 **90%** — Quality Gate 待 CI 重扫验证（BLOCKER 已清除，S1192/S3776 部分修复） |
...
| CI | `ci.yml` Sonar 扫描 + `sonar-project.properties` exclusion 已扩展 |
| **Quality Gate** | **待重扫** — BLOCKER `genhash` + `secrets:S8215` 已修复；S1656 自赋值已修复；S1192 部分提取 |
| Sonar 可靠性 | 预期 **A**（1 Bug `S1656` 已修复） |
| Sonar 安全性 | 预期 **A**（2 Vulnerability 已修复/排除） |
| Sonar 可维护性 | A（~30 Code Smell 待扫） |
| 技术债 | ~300 min（预估）；新代码重复率 **<3%** ✅ |
...
1. ✅ T-413/T-412/T-002 — finance 自赋值修复 + exclusion 扩展 + genhash const
2. ✅ T-422 — PowerShell `Write-Host` → `Write-Output`
3. ✅ T-420/T-421 — `confirmShip` 外提 + `globalThis` + unused import
4. **待 CI 重扫** 验证 QG 转绿
5. ~~T-402~~ — 剩余 `go:S1192`/`S3776` ~8 处（inventory/outbox/transport），低优先级
